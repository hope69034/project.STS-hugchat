...
def chatchat(a):
  try:
    global output2
    output = chatbot.chat(a).split('.')
    outputNew=''
    for j,i in enumerate(output) :
        outputNew += i + ' '
        if j==1:
            break
    output = translate_english_to_korean(outputNew) # Translate
    print('Bot:',output)
...
def trans():  
  ...
  while True: 
    ...
    with sr.Microphone() as source:
      ...
      text = r.recognize_google(audio, language='ko')  
      if text=='대화 모드'or text == '대화모드':
         print(text)
         print('<<<<<<<<<<<<<<<< 대화 모드입니다. >>>>>>>>>>>>>>>>')
         pygame.init()
         pygame.mixer.init()
         pygame.mixer.music.load('./say3.mp3')
         pygame.mixer.music.play()
         while pygame.mixer.music.get_busy():
             continue
          # Close the audio file
...
def main():
  ...
  while True: 
    ...
    try:
        text = r.recognize_google(audio, language='ko')  
        print(text)
        if text=='번역 모드'or text == '번역모드'  :
          pygame.init()
          pygame.mixer.init()
          pygame.mixer.music.load('./say2.mp3')
          pygame.mixer.music.play()
          while pygame.mixer.music.get_busy():
              continue
          # Close the audio file
